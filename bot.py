        ...
        for attempt_email in range(1, 4):
            ...
            if email_type:
                ...
                for attempt_code in range(1, 4):
                    response_code = await client.wait_for(const.message, check=lambda m: m.author == member and m.channel == canal_autenticacao, timeout=900)
            ...
            response_email = await client.wait_for(const.message, check=lambda m: m.author == member and m.channel == canal_autenticacao, timeout=900)
...
async def criar(ctx):
    ...
    try:
        ...
        if member_role != const.aluno:
            ...
            while True:
                response_channel_name = await client.wait_for(const.message, check=lambda m: m.author == ctx.author and isinstance(m.channel, discord.DMChannel), timeout=120)  # Aguarda a mensagem do usuário por 60 segundos
        ...
        while True:
            while True:
                response_number_vacancies = await client.wait_for(const.message, check=lambda m: m.author == ctx.author and isinstance(m.channel, discord.DMChannel), timeout=120)
        ...
        overwrites = {
            ...
            ctx.author: discord.PermissionOverwrite(connect=True, mute_members=True, deafen_members=True, view_channel=True),
